...
The objective of this study is to implement the optimal imputation techniques that addresses Neumann's data's missingness by region and to develop future alternatives of country conflict landscape using an iterative imputation style explanatory forecasting method. 
...
This chapter discusses the results and analysis of applying the methodology outlined in Chapter 3. Each variable of interests' regression models are analzyed in Section 4.2. Section 4.3 discusses the results from the data driven future alternatives of country conflict transitions. Section 4.3 discusses the specified scenarios and resulting impact on the conflict in the Arab region. Section 4.5 covers the alterternative futures of what if scenarios in the South East Asia grouping of countries.
...
#kableExtra::kable(text_tbl, "latex", booktabs = T) %>%
#  kable_styling(position = "center")
weak_mod_table <- xtable::xtable(SE_weak_models, caption = 'Studentized Residual Identified Weak SE Asia Individual Regression Models', label = 'table:weakSEmodels', align = "llc")
print(weak_mod_table, comment = F, include.rownames=FALSE, scalebox = 0.9, sanitize.text.function = function(x){x}, table.placement = "H")
```

Table \@ref(tab:weakSEmodels) shows that Singapore and Timor-Leste both contained studentized residuals greater than the standard cutoff for otulier analysis. Both of these nations, despite being poorly explained by the models for two variables each, had different conflcit trends in the alternative futures driven by unrestricted variable regression models. Singapore experienced an average percentage increase in all three conflict measurements, but Timor-Leste had simliar conflict reates of transitions and likelihoods as historically observed. Due to this difference in conflcit behavior between two countries that were seemingly poor fits by the most number of variables' models, this research cannot clearly attribute any conflict tendancies to countries that are notably undefined by the South East East Asia's variables of interest. 
...
In the Arab region, the data alternative futures driven by the relationships defined by the data predicted an overall ten year increase in conflicts compared to the observed past ten years of data. On average the future alternatives subject to no regional scenarios predicted 134.2 conflicts to arise in the next ten years opposed to 98 istances of conflict observed across the Arab region's nations. Conflict in the region was overall predicted to rise, and Bahrain, Kuwait, Oman, and Tunisia were the four nations of the sixteen in the region that had higher likelihood of conflict than the five repition average of regional conflict likelihood percentage change (82.716\%) making them the nations with conflict behavior least like their past. The only measure that the purely data driven future alternatives didn't predict would have an average percentage increase across the region was there was a small predicted average percentage decrease (7.451\%) of the most recent three years of conflict likelihoods compared to the observed most recent conflict likelihoods. The Arab region in the short term was less likely to be in conflict but in the long term the region saw an 82.716\% average increase in the likelihood of conflict. 

The South East Asia region experienced different conflict trends than the Arab region's general rise in instances of violent conflict. The alternative futures driven by unrestricted relationships of the variables of interest predicted that across all countries and repitions there would be 97.2 conflicts in the next ten years. This is a serious downturn from the past ten years of observed data's 136 conflicts in the region. Neither region's ten year future conflict estimates had standard deviations greater than eight meaning they're convincingly different than the past total conflict counts. Depsite having a small average percentage change in conflict likelihood (18.989\%), the alarming average percentage increase of likelihood of conflict that were greater than a 100\% increase occurred Bhutan, Samoa, and the Solomon Islands. The average rates of transitions in the region rose by 103.061\% meaning nations are transitioning conflict statuses much more often than historically on top of the general decrease of conflicts.
